...
func _on_jump_state_physics_processing(delta: float) -> void:
	...
	velocity.y += stat_component.gravity * delta
...
func _handle_jump_input() -> void:
	...
	var velocity: Vector2 = owner.velocity
	# 一段跳
	if stat_component.can_jump:
		DebugSystem.printDebug("跳跃", owner)
		stat_component.can_jump = false
		velocity.y = -stat_component.jump_speed
		owner.velocity = velocity
		return	
	# 二段跳
	if stat_component.can_double_jump:
		DebugSystem.printDebug("二段跳", owner)
		stat_component.is_double_jumping = true
		stat_component.can_double_jump = false
		velocity.y = -stat_component.double_jump_speed
		owner.velocity = velocity
...
func _update_double_jump_availability(velocity: Vector2) -> void:
	...
	if velocity.y > 0 and not stat_component.can_jump and not stat_component.is_double_jumping:
