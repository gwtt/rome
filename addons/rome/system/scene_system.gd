...
func transitionToScene(nextScene: PackedScene, pauseSceneTree: bool = true, animate: bool = animateDefault) -> void:
	if not is_instance_valid(nextScene):
		DebugSystem.printError(str("transitionToScene(): Invalid scene: ", nextScene), logName)
		return
	...
	if ongoingTransitionScene == nextScene:
		DebugSystem.printWarning(str("transitionToScene() called for the same scene during a transition: ", nextScene, " ", nextScene.resource_path), logName)
		return

	var sceneBeforeTransition: Node = sceneTree.current_scene
	DebugSystem.printAutoLoadLog(str("transitionToScene(): ", sceneBeforeTransition, " → ", nextScene, " ", nextScene.resource_path))
	...
	if DebugSystem.shouldPrintDebugLogs: DebugSystem.printDebug(str("SceneTree.current_scene: ", sceneTree.current_scene), logName)
...
func pushSceneToStack(scenePath: String) -> int:
	if scenePath.is_empty():
		DebugSystem.printWarning("pushSceneToStack(): Path empty!", logName)
	...
	if not sceneStack.is_empty() and sceneStack[sceneStack.size() - 1] == scenePath:
		DebugSystem.printWarning("pushSceneToStack(): Scene already on top of stack: " + scenePath, logName)
	...
	if DebugSystem.shouldPrintDebugLogs:
		DebugSystem.printDebug(str("pushSceneToStack(): ", scenePath, " → ", sceneStack.size(), ": ", sceneStack), logName)
...
func pushCurrentSceneToStack() -> int:
	...
	if not is_instance_valid(currentScene):
		DebugSystem.printWarning("pushCurrentSceneToStack(): No valid current scene", logName)
		return sceneStack.size()
	...
	if not currentScenePath.is_empty():
		...
		DebugSystem.printWarning("pushCurrentSceneToStack(): Cannot get path for current scene", logName)
...
func popSceneFromStack(pauseSceneTree: bool = true, animate: bool = animateDefault) -> PackedScene:
	...
	if sceneStack.is_empty(): # Can't pop if there are no scenes on the stack.
		DebugSystem.printWarning("popSceneFromStack(): sceneStack is empty!", logName)
		return null
	...
	if previousSceneFromStack:
		DebugSystem.printAutoLoadLog(str("popSceneFromStack() → ", previousScenePathFromStack, " → stack size: ", sceneStack.size()))
	else:
		DebugSystem.printError("popSceneFromStack(): Cannot load path: " + previousScenePathFromStack, logName)
		return null
	...
	if sceneTree.current_scene:
		...
		if not scenePathAfterTransition == previousScenePathFromStack:
			DebugSystem.printWarning(str("SceneTree.current_scene.scene_file_path: ", scenePathAfterTransition, " != previousScenePathFromStack: ", previousScenePathFromStack), logName)
...
func instantiateSceneFromPath(path: String) -> Node:
	...
	if is_instance_valid(scene):
		...
		if is_instance_valid(instance): return instance
			...
			DebugSystem.printWarning(str("SceneManager.instantiateSceneFromPath(): Cannot instantiate ", scene, " from ", path))
	else:
		DebugSystem.printWarning("SceneManager.instantiateSceneFromPath(): Cannot load " + path)
...
func addSceneInstance(scene: PackedScene, parent: Node, position: Vector2 = Vector2.ZERO) -> Node:
	if scene == null:
		DebugSystem.printWarning(str("SceneManager.addSceneInstance(): scene is null!"))
		return null

	var newChild := scene.instantiate()

	if not is_instance_valid(newChild):
		DebugSystem.printWarning(str("SceneManager.addSceneInstance(): Cannot instantiate ", scene))
		return null
