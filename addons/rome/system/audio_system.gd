...
@export var audios_omni: Array[AudioSystemOmni] = []
	...
	set(value):
		...
		if is_instance_valid(target_parent_audios_2d):
			for audio in audios_2d:
				var audio_controller: AudioSystemController2D = _get_audio_controller_2d(audio.audio_name)
...
@export var audios_2d: Array[AudioSystem2D] = []
	...
	set(value):
		...
		if is_instance_valid(target_parent_audios_3d):
			for audio in audios_3d:
				var audio_controller: AudioSystemController3D = _get_audio_controller_3d(audio.audio_name)
				if is_instance_valid(audio_controller):
					if not audio_controller.is_inside_tree():
...
@export var audios_3d: Array[AudioSystem3D] = []
...
func play_audio_omni(audio_name: String) -> void:
	var audio: AudioSystemControllerOmni = _validate_audio_omni(audio_name)
	if not audio:
		return
...
func play_audio_2d(audio_name: String) -> void:
	var audio: AudioSystemController2D = _validate_audio_2d(audio_name)
	if not audio:
		return
...
func play_audio_3d(audio_name: String) -> void:
	var audio: AudioSystemController3D = _validate_audio_3d(audio_name)
	if not audio:
		return
...
func is_playing_omni(audio_name: String) -> bool:
	var audio: AudioSystemOmni = get_audio_omni(audio_name)
	return audio._owner.playing if audio else false


func is_playing_2d(audio_name: String) -> bool:
	var audio: AudioSystem2D = get_audio_2d(audio_name)
	return audio._owner.playing if audio else false


func is_playing_3d(audio_name: String) -> bool:
	var audio: AudioSystem3D = get_audio_3d(audio_name)
	return audio._owner.playing if audio else false


## Get audio 3D (AudioSystem3D)
func get_audio_3d(_audio_name: String) -> AudioSystem3D:
	for aud in audios_3d:
		if aud.audio_name == _audio_name:
			return aud
	push_warning("AudioSystem3D %s not find."%_audio_name)
	return null


## Get audio Omni (AudioSystemOmni)
func get_audio_omni(_audio_name: String) -> AudioSystemOmni:
	for aud in audios_omni:
		if aud.audio_name == _audio_name:
			return aud
	push_warning("AudioSystemOmni %s not find."%_audio_name)
	return null


## Get audio 2D (AudioSystem2D)
func get_audio_2d(_audio_name: String) -> AudioSystem2D:
	for aud in audios_2d:
		if aud.audio_name == _audio_name:
			return aud
	push_warning("AudioSystem2D %s not find."%_audio_name)
	return null
...
func _init_audios_omni() -> void:
	for audio_omni in audios_omni:
		...
		var new_audio_manager_controller_omni: AudioSystemControllerOmni = AudioSystemControllerOmni.new(
...
func _init_audios_2d() -> void:
	for audio_2d in audios_2d:
		...
		var new_audio_manager_controller_2d: AudioSystemController2D = AudioSystemController2D.new(
...
func _init_audios_3d() -> void:
	for audio_3d in audios_3d:
		...
		var new_audio_manager_controller_3d: AudioSystemController3D = AudioSystemController3D.new(
...
func _setup_audio_properties_omni(audio: AudioStreamPlayer, a: AudioSystemOmni) -> void:
...
func _setup_audio_properties_2d(audio: AudioStreamPlayer2D, a: AudioSystem2D) -> void:
	audio.stream = a.audio_stream
	audio.volume_db = a.volume_db
	audio.pitch_scale = a.pitch_scale
...
func _setup_audio_properties_3d(audio: AudioStreamPlayer3D, a: AudioSystem3D) -> void:
	audio.stream = a.audio_stream
	audio.volume_db = a.volume_db
...
func _validate_audio_3d(_audio_name: String) -> AudioSystemController3D:
	var audio: AudioSystemController3D = _get_audio_controller_3d(_audio_name)
	if not audio:
		push_warning("AudioSystem3D name (%s) not found." % _audio_name)
	return audio


func _validate_audio_omni(_audio_name: String) -> AudioSystemControllerOmni:
	var audio: AudioSystemControllerOmni = _get_audio_controller_omni(_audio_name)
	if not audio:
		push_warning("AudioSystemOmni name (%s) not found." % _audio_name)
	return audio


func _validate_audio_2d(_audio_name: String) -> AudioSystemController2D:
	var audio: AudioSystemController2D = _get_audio_controller_2d(_audio_name)
	if not audio:
		push_warning("AudioSystem2D name (%s) not found." % _audio_name)
	return audio


func _setup_timer_omni(_audio_name: String) -> Timer:
	var audio: AudioSystemControllerOmni = _get_audio_controller_omni(_audio_name) as AudioSystemControllerOmni
...
func _setup_timer_2d(_audio_name: String) -> Timer:
	var audio: AudioSystemController2D = _get_audio_controller_2d(_audio_name) as AudioSystemController2D
	audio.timer.one_shot = not audio.loop
	audio.timer.wait_time = max(audio.duration, 0.00001)
	if not audio.is_timer_connected:
...
func _setup_timer_3d(_audio_name: String) -> Timer:
	var audio: AudioSystemController3D = _get_audio_controller_3d(_audio_name) as AudioSystemController3D
	audio.timer.one_shot = not audio.loop
	audio.timer.wait_time = max(audio.duration, 0.00001)
	if not audio.is_timer_connected:
...
func _on_timer_timeout_omni(_audio: AudioSystemControllerOmni, _audio_name: String, cb: Callable) -> void:
	if _audio.loop:
		cb.call()
	else:
...
func _on_timer_timeout_2d(_audio: AudioSystemController2D, _audio_name: String, cb: Callable) -> void:
	if _audio.loop:
		cb.call()
	else:
		if not _audio.is_playlist:
...
func _on_timer_timeout_3d(_audio: AudioSystemController3D, _audio_name: String, cb: Callable) -> void:
	if _audio.loop:
		cb.call()
	else:
		if not _audio.is_playlist:
...
func _get_audio_controller_omni(_audio_name: String) -> AudioSystemControllerOmni:
	return audios_manager_controller_omni.get(_audio_name, null) as AudioSystemControllerOmni


func _get_audio_controller_2d(_audio_name: String) -> AudioSystemController2D:
	return audios_manager_controller_2d.get(_audio_name, null) as AudioSystemController2D


func _get_audio_controller_3d(_audio_name: String) -> AudioSystemController3D:
	return audios_manager_controller_3d.get(_audio_name, null) as AudioSystemController3D
...
func _warning_audio(_audio: Variant) -> void:
	...
	if _audio.duration <= 0.0 and not (_audio.is_randomizer or _audio.is_interactive or _audio.is_synchronized or _audio.is_playlist):
		push_warning("AudioSystem duration cannot be less than or equal to zero. Check START_TIME, END_TIME. (%s)" % _audio.audio_name)
	if _audio.use_clipper and not (_audio.is_randomizer or _audio.is_interactive or _audio.is_synchronized or _audio.is_playlist) and _audio.start_time > _audio.end_time:
		push_warning("Start time cannot be greater than end time in AudioSystem resource: (%s)" % _audio.audio_name)


func _check_audio(_audio: Variant) -> bool:
	if not _audio or not _audio.audio_stream:
		push_warning("AudioSystem resource or its stream is not properly defined.")
		return false
	if not (_audio.is_randomizer or _audio.is_interactive or _audio.is_synchronized or _audio.is_playlist) and _audio.start_time > _audio.end_time:
		push_warning("AudioSystem start time cannot be greater than end time for '%s'. AudioSystemResource deleted from ManagerList." % _audio.audio_name)
		return false
	return true


#*****************************************************************************
class AudioSystemControllerOmni extends AudioStreamPlayer:
	var timer: Timer
...
#*****************************************************************************
class AudioSystemController2D extends AudioStreamPlayer2D:
	var timer: Timer
	var start_time: float
	var duration: float
	var use_clipper: bool
...
#*******************************************************************
class AudioSystemController3D extends AudioStreamPlayer3D:
	var timer: Timer
	var start_time: float
	var duration: float
	var use_clipper: bool
